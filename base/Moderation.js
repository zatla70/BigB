...
class Moderation extends Command {
  ...
  async buildModLog(client, guild, action, target, mod, reason) {
    const modChan = this.client.serverConfig[message.guild.id].modLogChannel
    const prefix = this.client.config.prefix;
    const caseNumber = await this.caseNumber(client, guild.channels.find('name', modChan));
    const thisAction = this.actions[action];

    if (reason.length < 1) reason = `Awaiting moderator's input. Use ${prefix}reason ${caseNumber} <reason>.`;
